const domain = "https://boxwood-yen-382002.ue.r.appspot.com";
...
export const register = (credential, asHost) => {
  ...
  return fetch(registerUrl, {
    ...
    if (response.status !== 200) throw Error("FAIL TO REGISTER!");
  });
};
...
export const bookStay = (data) => {
    ...
    return fetch(bookStayUrl, {
      ...
      if (response.status !== 200) throw Error("FAILED TO BOOK RESERVATION!");
    });
};
...
export const getReservationsByStay = (stayId) => {
    ...
    return fetch(getReservationByStayUrl, {
      ...
      if (response.status !== 200) throw Error("FAILED TO GET RESERVATIONS BY STAY!");
      return response.json();
    });
};
